...
let editingId=null
function loadData(){
    const saved=localStorage.getItem('samarthflowTasks')
    if(saved) tasks = JSON.parse(saved);
    else
        tasks = [
            {
                id:1,
                title: "Evaluate the addition and deletion of users IDs",
                status: "pending",
                priority: "low",
                completed: false,
            },
...
function updateGreeting(){
    ...
    else if(hour>=18) greet="Goodnight";
    document.getElementById("greeting").textContent=`${greet}, Samarth` 
}

function savedData(){
    localStorage.setItem("samarthflowTasks", JSON.stringify(tasks))
}
        ...
        ? onHold
            ...
            <div class="task-item">
                            ...
                            <div class="priority-badge priority-${t.priority}">
                                <i class="fi fi-sr-circle"></i> ${t.priority.charAt(0).toUpperCase() + t.priority.slice(1)}
                            </div>
                            ...
                            <button class="icon-btn" style="width: 30px; height:30px;" onclick="deleteTask(${t.id})">
                                <i class="fi fi-sr-trash" style="font-size: 12px;"></i>
                            </button>
                        </div>
    `).join("")
